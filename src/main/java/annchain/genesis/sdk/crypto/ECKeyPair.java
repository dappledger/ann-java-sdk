...
public class ECKeyPair {
     ...
     * @return  An {@link ECDSASignature} of the hash
     */
    public String anChainSign(byte[] transactionHash) {
        ECDSASigner signer = new ECDSASigner(new HMacDSAKCalculator(new SHA256Digest()));
        ...
        int rLenght= r.toByteArray().length;
        int sLength = s.toByteArray().length;
        byte[] result = new byte[r.toByteArray().length+s.toByteArray().length];
        System.arraycopy(r.toByteArray(),0,result,0,r.toByteArray().length);
        System.arraycopy(s.toByteArray(),0,result,r.toByteArray().length,s.toByteArray().length);
